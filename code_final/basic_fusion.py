...
def predict_majority_rule(predict_ensemble):
    default = None
    RR_predict = predict_ensemble[0]
    WVL_predict = predict_ensemble[1]
    HOS_predict = predict_ensemble[2]
    myDesc_predict = predict_ensemble[3]
    dim = len(RR_predict)
    predictions_rule = np.zeros(dim)
    for index in range(dim):
        seq = []
        seq.append(RR_predict[index])
        seq.append(WVL_predict[index])
        seq.append(HOS_predict[index])
        seq.append(myDesc_predict[index])
        candidate = default
        count = 0
        for e in seq:
            if count != 0:
                ...
                candidate = e
                count = 1
        predictions_rule[index] = candidate if seq.count(candidate) >= len(seq) // 2 else default
    return predictions_rule

def probability_majority_rule(predict_ensemble):
    default=None
    RR_predict=predict_ensemble[0]
    WVL_predict=predict_ensemble[1]
    HOS_predict=predict_ensemble[2]
    myDesc_predict=predict_ensemble[3]
    end_predict=predict_ensemble[4]
    dim=len(RR_predict)
    predictions_rule = np.zeros(dim)
    for index in range(dim):
        seq=[]
...
def end_majority_rule(predict_ensemble):
    default = None
    RR_predict = predict_ensemble[0]
    WVL_predict = predict_ensemble[1]
    dim = len(RR_predict)
...
def main():
    ...
    # Load gt labelso
    eval_labels = np.loadtxt('C:/Users/Matteo/Desktop/data_mining_prog/ecg-classification-master/python/mit_db/' + DS + '_labels.csv')

    # Configuration
    results_path = 'C:/Users/Matteo/Desktop/data_mining_prog/ecg-classification-master/python/results/ovo/MLII/'
    ...
    if DS == 'DS2':     
        ...
        model_HBF           = results_path + oversamp + 'rm_bsln/' + 'maxRR/' + 'hbf5/'           + 'weighted/' + 'C_0.001' + '_decision_ovo.csv'
